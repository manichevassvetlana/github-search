<template>
    <div class="container">
        <div id="profile" v-if="isLoaded" style="margin-top: 5%">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <img class="thumbnail avatar" style="width: 100%;"
                                 :src="user.avatar_url">
                            <a target="_blank" class="btn btn-primary btn-block" :href="user.html_url">View on
                                GitHub</a>
                        </div>
                        <div class="col-md-9">
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item"><strong>Member
                                        Since: </strong> {{(user.created_at).split('T')[0]}}</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="row" v-if="isFollowersLeft" style="margin-bottom: 5%; float:right">
            <div class="col-md-5"></div>
            <div class="col-md-5"></div>
            <div class="col-md-2">
                <button class="btn btn-info" @click="fetchUserFollowers">More</button></div>
        </div>
    </div>
...
<script>
    export default {
        props: ['name'], // Requirement: the name of the user
        data() {
            return {
                searchName: this.name, // Search query
        ...
        methods: {
            ...
            fetchUser(){
                if (this.searchName.length >= 1) {
                    this.isLoaded = false;
                    axios.get('/search/user/' + this.searchName).then(response => {
            ...
            fetchUserFollowers(){
                if (this.isFollowersLeft) {
                    axios.post("/search/followers",
                        {link: this.user.followers_url, page: this.currentFollowerPage, count: this.countPerPage}
                    ).then(response => {
